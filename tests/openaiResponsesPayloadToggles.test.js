...
describe('openai responses payload toggles', () => {
  ...
  test('records gpt-image-2 when non-stream responses output contains image generation call', async () => {
    unifiedOpenAIScheduler.selectAccountForApiKey.mockResolvedValue({
      accountId: 'openai-1',
      accountType: 'openai'
    })
    ...
    axios.post.mockResolvedValue({
      ...
      data: {
        ...
        output: [{ type: 'image_generation_call', id: 'ig_123' }],
        usage: {
          input_tokens: 20,
          output_tokens: 5,
          total_tokens: 25
    ...
    expect(apiKeyService.recordUsage).toHaveBeenCalled()
    expect(apiKeyService.recordUsage.mock.calls[0][5]).toBe('gpt-image-2')
  })
