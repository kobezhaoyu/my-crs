...
describe('apiKeyService openai responses config', () => {
  ...
  test('recordUsageWithDetails honors fixed image pricing overrides', async () => {
    const result = await apiKeyService.recordUsageWithDetails(
      'key-1',
      {
        input_tokens: 120,
    ...
    expect(serviceRatesService.getService).not.toHaveBeenCalled()
    expect(result.realCost).toBeCloseTo(0.08, 10)
    expect(result.ratedCost).toBeCloseTo(0.08, 10)
    ...
    expect(redis.incrementDailyCost).toHaveBeenCalledWith('key-1', 0.08, 0.08)
    expect(redis.addUsageRecord).toHaveBeenCalledWith(
      'key-1',
      expect.objectContaining({
        model: 'gpt-image-2',
