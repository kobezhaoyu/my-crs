...
function getImageSessionId(req) {
  return (
...
function isClientConnectionClosed(req, res) {
  return req.aborted || (res.destroyed && !res.writableEnded)
}

function getImageRelayHealthConfig() {
  const upstreamHealthCheckTimeoutMs = Math.max(
    1000,
    parseInt(config.imageRelay?.upstreamHealthCheckTimeoutMs, 10) || 5000
  )
  const totalHealthCheckTimeoutMs = Math.max(
    upstreamHealthCheckTimeoutMs,
    parseInt(config.imageRelay?.totalHealthCheckTimeoutMs, 10) || 30000
  )

  return {
    upstreamHealthCheckTimeoutMs,
    totalHealthCheckTimeoutMs
  }
}

function createImageServiceUnavailableError(
  message = '生图服务异常',
  ...
  error.isImageServiceUnavailable = true
  return error
}

function isRetryableImageRelayError(error) {
  const status = error?.statusCode || error?.response?.status || 500
  return status === 403 || status === 429 || status >= 500 || error?.code === 'ECONNABORTED'
}

function normalizeImageGenerationBody(body = {}) {
  const normalized = body && typeof body === 'object' ? { ...body } : {}
  if (!normalized.model) {
    normalized.model = IMAGE_GENERATION_MODEL
  }

  // Shared Responses/Codex clients may send fields that the Images API rejects.
    ...
    if (!isStream) {
      ...
      try {
        ...
        if (modelToRecord === IMAGE_GENERATION_MODEL && modelToRecord !== actualModel) {
          logger.info('[image] Detected image generation in non-stream Responses output')
        }

        logger.debug(`📊 Non-stream response - Model: ${modelToRecord}, Usage:`, usageData)
        ...
        if (usageData) {
          ...
          logger.info(
            `📊 Recorded OpenAI non-stream usage - Input: ${totalInputTokens}(actual:${actualInputTokens}+cached:${cacheReadTokens}), Output: ${outputTokens}, Total: ${usageData.total_tokens || totalInputTokens + outputTokens}, Model: ${modelToRecord}`
          )
    ...
    upstream.data.on('end', async () => {
      ...
      if (!usageReported && usageData) {
        try {
          ...
          const baseModelToRecord = actualModel || upstreamRequestedModel || 'gpt-4'
          ...
          if (hasImageGeneration && modelToRecord !== baseModelToRecord) {
            logger.info('[image] Detected image generation in stream Responses output')
          }
...
const handleImageGeneration = async (req, res) => {
  try {
    ...
    req.body = normalizeImageGenerationBody(req.body)

    const requestedModel = req.body?.model || IMAGE_GENERATION_MODEL
    const sessionId = getImageSessionId(req)
    const sessionHash = sessionId
      ? crypto.createHash('sha256').update(sessionId).digest('hex')
      : null
    const { upstreamHealthCheckTimeoutMs, totalHealthCheckTimeoutMs } = getImageRelayHealthConfig()
    const healthCheckDeadlineAt = Date.now() + totalHealthCheckTimeoutMs

    const attemptedAccountIds = []
    let lastError = null
    let attempt = 0

    while (Date.now() < healthCheckDeadlineAt) {
      let selection = null
      try {
        selection = await unifiedOpenAIScheduler.selectOpenAIResponsesAccountForApiKey(
          apiKeyData,
          sessionHash,
          requestedModel,
          attemptedAccountIds
        ...
        if (selectionError?.statusCode === 402) {
      ...
      try {
        attempt += 1
        account = await openaiResponsesAccountService.getAccount(selection.accountId)
        if (!account || !account.apiKey) {
          logger.warn(
            `[image] Skipping account ${selection.accountId} because apiKey is missing for image generation`
          )
          attemptedAccountIds.push(selection.accountId)
          lastError = createImageServiceUnavailableError(
            `Image account ${selection.accountId} is missing apiKey`
          )
          continue
        }

        logger.info(
          `[image] Using OpenAI-Responses image relay for account: ${account.name} (attempt ${attempt})`
        )

        const remainingHealthBudgetMs = healthCheckDeadlineAt - Date.now()
        if (remainingHealthBudgetMs <= 0) {
          break
        }

        const healthResult = await openaiResponsesRelayService.checkImageUpstreamHealth(account, {
          timeoutMs: Math.min(upstreamHealthCheckTimeoutMs, remainingHealthBudgetMs)
        })

        if (!healthResult?.healthy) {
          attemptedAccountIds.push(selection.accountId)
          lastError = createImageServiceUnavailableError(
            `Image upstream health check failed for ${account.name}`
          )
          logger.warn(
            `[image] Health check failed for account ${account.name}; trying next image account`
          )
          continue
        }

        return await openaiResponsesRelayService.handleImageGenerationRequest(
          req,
          res,
          account,
          apiKeyData,
        ...
        if (relayError?.isClientAbort || isClientConnectionClosed(req, res)) {
          logger.info('[image] Client disconnected during image generation; stopping retries')
          return null
        }
        ...
        if (!isRetryableImageRelayError(relayError) || res.headersSent) {
          throw relayError
        }

        const status = relayError.statusCode || relayError.response?.status || 500
        logger.warn(
          `[image] Upstream account ${account?.name || selection.accountId} failed with status ${status}; trying next image account`
        )
      }
    }

    logger.error('[image] All image upstreams failed health check or request relay', {
      attemptedAccountIds,
      lastError: lastError?.message || null
    })

    throw lastError?.isImageServiceUnavailable ? lastError : createImageServiceUnavailableError()
  } catch (error) {
    if (error?.isClientAbort || isClientConnectionClosed(req, res)) {
      logger.info('[image] Client disconnected before image generation completed')
      return null
    }

    logger.error('OpenAI image generation proxy failed:', error)
    const status = error.statusCode || error.response?.status || 500
    const message = error.isImageServiceUnavailable
      ? '生图服务异常'
      : getSafeMessage(error.response?.data?.error?.message || error.message || error)

    if (!res.headersSent) {
      return res.status(status).json({
        error: {
          message,
          type: status === 402 ? 'insufficient_quota' : 'api_error',
          code:
            status === 402
              ? 'no_image_account_available'
              : error.errorCode || 'image_generation_failed'
        }
      })
    }
  }
...
// OpenAI Images API 路由，和 Codex/Responses 共用 /openai base_url。
