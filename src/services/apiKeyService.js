...
class ApiKeyService {
  ...
  async recordUsageWithDetails(
    ...
    try {
      ...
      const totalTokens = inputTokens + outputTokens + cacheCreateTokens + cacheReadTokens
      const fixedCostUsd = Number(finalizedRequestMeta?.fixedCostUsd)
      const hasFixedCostOverride = Number.isFinite(fixedCostUsd) && fixedCostUsd >= 0
      ...
      if (hasFixedCostOverride) {
        costInfo = {
          totalCost: fixedCostUsd,
          inputCost: 0,
          outputCost: 0,
          cacheCreateCost: 0,
          cacheReadCost: 0,
          ...
          isLongContextRequest: false,
          usedFallbackPricing: false,
          pricingSource: finalizedRequestMeta?.fixedPricingSource || 'fixed'
        }
      } else {
        try {
          const CostCalculator = require('../utils/costCalculator')
          ...
          costInfo = {
            ...
            cacheCreateCost:
              Number(
                costs.cacheCreate ?? costs.cacheWrite ?? calculatedCost?.cacheCreateCost ?? 0
              ) || 0,
      ...
      if (!hasFixedCostOverride && realCostWithDetails > 0) {
        const service = serviceRatesService.getService(accountType, model)
