...
class UnifiedOpenAIScheduler {
  ...
  _isImageGenerationModel(model) {
    return typeof model === 'string' && model.toLowerCase().startsWith('gpt-image')
  }

  _hasStrongImageAccountSignal(account) {
    const name = String(account?.name || '').toLowerCase()
    const signalPattern = /gpt-image|(^|[^a-z0-9])(images?|img)([^a-z0-9]|$)/
    return signalPattern.test(name)
  }

  _supportsOpenAIImagesApi(account, requestedModel = null) {
    if (!this._isImageGenerationModel(requestedModel)) {
      return true
    }

    const baseApi = account?.baseApi || ''
    const name = String(account?.name || '').toLowerCase()
    const supportedModels = String(account?.supportedModels || account?.models || '').toLowerCase()
    const nameImageSignal = /gpt-image|(^|[^a-z0-9])(images?|img)([^a-z0-9]|$)/.test(name)
    const imageSignal = this._hasStrongImageAccountSignal({
      name,
      supportedModels
    })
    if (!baseApi) {
    ...
    try {
      ...
      if (host.includes('aipluspro.xyxw.top')) {
        return true
      }
      if (!path || path === '/v1') {
        // 普通 FY 组未开启生图权限，只允许 FY-img 这类带正向标识的账号。
        if (host.includes('ferribuy.asia')) {
          return nameImageSignal
        }
        if (imageSignal) {
          return true
        }
      ...
      // Codex/Responses-only relays accept /responses but not OpenAI Images API paths.
      if (path.includes('/codex') || path.endsWith('/responses')) {
        return false
      }
      if (host.includes('ferribuy.asia')) {
        return nameImageSignal
  ...
  // [image] 选择 OpenAI-Responses 账户用于 Images API，避免误用 ChatGPT/Codex OAuth 账户。
  async selectOpenAIResponsesAccountForApiKey(
    apiKeyData,
    sessionHash = null,
    requestedModel = null,
    excludeAccountIds = []
    ...
    try {
      ...
      if (apiKeyData.openaiAccountId) {
        if (apiKeyData.openaiAccountId.startsWith('group:')) {
          const groupId = apiKeyData.openaiAccountId.replace('group:', '')
          return await this.selectOpenAIResponsesAccountFromGroup(
            groupId,
            sessionHash,
            requestedModel,
            excludeAccountIds
        ...
        if (!apiKeyData.openaiAccountId.startsWith('responses:')) {
          const error = new Error('Dedicated OpenAI account is not available for image generation')
        ...
        if (excluded.has(accountId)) {
          const error = new Error(
            'Dedicated OpenAI-Responses account was already attempted for image generation'
          )
          error.statusCode = 402
          throw error
        }
        const account = await openaiResponsesAccountService.getAccount(accountId)
        const available = await this._isAccountAvailable(accountId, 'openai-responses')
        if (!account || !available || !this._supportsOpenAIImagesApi(account, requestedModel)) {
          const error = new Error(
            'Dedicated OpenAI-Responses account is not available for image generation'
          )
          error.statusCode = account ? 403 : 404
          throw error
        }
        await this.updateAccountLastUsed(accountId, 'openai-responses')
        return { accountId, accountType: 'openai-responses' }
      }
      ...
      if (sessionHash) {
        ...
        if (
          ...
          !excluded.has(mappedAccount.accountId)
        ) {
          const account = await openaiResponsesAccountService.getAccount(mappedAccount.accountId)
          const available = await this._isAccountAvailable(
            mappedAccount.accountId,
            'openai-responses'
          )
          if (available && this._supportsOpenAIImagesApi(account, requestedModel)) {
            await this._extendSessionMappingTTL(sessionHash)
            await this.updateAccountLastUsed(mappedAccount.accountId, 'openai-responses')
            return mappedAccount
          }
        }
        if (mappedAccount) {
      ...
      const availableAccounts = await this._getAllAvailableOpenAIResponsesAccounts(
        apiKeyData,
        requestedModel,
        excludeAccountIds
      )
      if (availableAccounts.length === 0) {
        const error = new Error('No available OpenAI-Responses accounts for image generation')
        error.statusCode = 402
        throw error
      }

      const selectedAccount = sortAccountsByPriority(availableAccounts)[0]
      if (sessionHash) {
        await this._setSessionMapping(sessionHash, selectedAccount.accountId, 'openai-responses')
      }
      await this.updateAccountLastUsed(selectedAccount.accountId, 'openai-responses')

      logger.info(
        `[image] Selected OpenAI-Responses image account: ${selectedAccount.name} (${selectedAccount.accountId})`
      )
      ...
      logger.error('❌ Failed to select OpenAI-Responses image account:', error)
      throw error
    }
  }

  async selectOpenAIResponsesAccountFromGroup(
    groupId,
    sessionHash = null,
    requestedModel = null,
    excludeAccountIds = []
    ...
    if (sessionHash) {
      ...
      if (
        ...
        const account = isInGroup
          ? await openaiResponsesAccountService.getAccount(mappedAccount.accountId)
          : null
        const available =
          isInGroup && (await this._isAccountAvailable(mappedAccount.accountId, 'openai-responses'))
        if (available && this._supportsOpenAIImagesApi(account, requestedModel)) {
          await this._extendSessionMappingTTL(sessionHash)
          await this.updateAccountLastUsed(mappedAccount.accountId, 'openai-responses')
          return mappedAccount
    ...
    for (const memberId of memberIds) {
      ...
      const account = await openaiResponsesAccountService.getAccount(memberId)
      if (!account) {
        continue
      }
      if (!this._supportsOpenAIImagesApi(account, requestedModel)) {
        logger.debug(
          `⏭️ Skipping OpenAI-Responses account ${account.name} - Images API is not supported by baseApi`
        )
        continue
      }
      const available = await this._isAccountAvailable(memberId, 'openai-responses')
      if (!available) {
        continue
      }
      if (requestedModel && account.supportedModels && account.supportedModels.length > 0) {
    ...
    if (availableAccounts.length === 0) {
      const error = new Error(`No available OpenAI-Responses accounts in group ${group.name}`)
      error.statusCode = 402
      throw error
    }
  ...
  async _getAllAvailableOpenAIResponsesAccounts(
    apiKeyData,
    requestedModel = null,
    excludeAccountIds = []
  ) {
    const allAccounts = await this._getAllAvailableAccounts(apiKeyData, requestedModel)
    const excluded = new Set(excludeAccountIds || [])
    const availableAccounts = allAccounts.filter(
      (account) =>
        account.accountType === 'openai-responses' &&
        !excluded.has(account.accountId) &&
        this._supportsOpenAIImagesApi(account, requestedModel)
    )
